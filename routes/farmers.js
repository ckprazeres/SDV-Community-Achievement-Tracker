...
router.post('/add', isAuthenticated, function(req, res) {
	...
	.then(function(farmerName) {
		...
		req.flash('success_msg', 'New farmer was created! Your current farmer is now <strong class="farmer">' + farmerName + '</strong>.');
...
router.post('/delete/:id', isAuthenticated, function(req, res) {
	...
	if(req.user.dataValues.current_farmer_id == farmerID) {
		req.flash('error_msg', 'Can\'t delete the current farmer. Please switch to a different farmer before you delete <strong class="farmer">' + req.user.dataValues.current_farmer_name + '</strong>.');
		res.redirect('/user/farmers');
	}
	else {
...
router.post('/update/:id', isAuthenticated, function(req, res) {
	...
	.then(function(farmerName) {
		...
		req.flash('success_msg', 'Current farmer was successfully changed to <strong class="farmer">' + farmerName + '</strong>!');
		res.redirect('/user/farmers');
	})
});
